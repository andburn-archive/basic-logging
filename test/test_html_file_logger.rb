...
class TestHtmlFileLogger < Test::Unit::TestCase

	def setup
		@log_file_name = 'test/data/sample-log.html'
		...
		@logger = BasicLogging::HtmlFileLogger.new(@log_file_name)
		@message = 'simple message for HtmlFileLogger'
	end
	...
	private
		...
		def format_is_correct?(level, message, output)
			...
			css_class = level.downcase
			output.chomp =~ /<div\sclass="#{css_class}"><span\sclass="level">#{level}<\/span><span\sclass="datetime">\d{4}\-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}<\/span><span\sclass="message">#{message}<\/span><\/div>/
		end
